...
Процедура ПриОпределенииРегистровСДвижениями(Документ, РегистрыСДвижениями) Экспорт
	...
	Если ТипЗнч(Документ) = Тип("ДокументСсылка._ДемоПоступлениеТоваров") Тогда
		РегистрыСДвижениями.Вставить(Метаданные.РегистрыРасчета._ДемоОсновныеНачисления, "Регистратор");
	КонецЕсли;
...
Процедура ПриРасчетеКоличестваЗаписей(Документ, РассчитанноеКоличество) Экспорт
	...
	Если ТипЗнч(Документ) = Тип("ДокументСсылка._ДемоПоступлениеТоваров")
		И ПравоДоступа("Просмотр", Метаданные.РегистрыРасчета._ДемоОсновныеНачисления) Тогда
		
		Запрос = Новый Запрос(
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ ПЕРВЫЕ 1
		|	*
		|ИЗ
		|	РегистрРасчета._ДемоОсновныеНачисления КАК _ДемоОсновныеНачисления");
		
		КоличествоЗаписей = Запрос.Выполнить().Выгрузить().Количество();
		РассчитанноеКоличество.Вставить(СтрЗаменить(Метаданные.РегистрыРасчета._ДемоОсновныеНачисления.ПолноеИмя(), ".", "_"), 
			КоличествоЗаписей);
...
Процедура ПриПодготовкеНабораДанных(Документ, НаборыДанных) Экспорт
	...
	Если ТипЗнч(Документ) = Тип("ДокументСсылка._ДемоПоступлениеТоваров") Тогда
		...
		Для Индекс = 0 По НаборыДанных.Количество() - 1 Цикл
