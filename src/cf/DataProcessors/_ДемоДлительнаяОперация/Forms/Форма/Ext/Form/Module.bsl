...
Функция НачатьВыполнениеНаСервере(НомерОперации)
	...
	ОбщегоНазначения.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 Начало выполнения %2...'"), ТекущаяДатаСеанса(), НомерОперации));
