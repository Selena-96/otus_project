...
Процедура ВыгрузитьКонфигурациюВФайлы(ПапкаВыгрузки, Параметры)
	...
	ПараметрыЗапускаПрограммы = ОбщегоНазначенияКлиентСервер.ПараметрыЗапускаПрограммы(); // АПК:223 для возможности запуска на 3.0.1
...
Функция ОтправитьТекстыОграничений(ПараметрыОтправки)
	...
	ИмяСобытияВЖурналеРегистрации = НСтр("ru = 'Контроль изменения текстов RLS'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПолучениеФайловИзИнтернета") Тогда
		...
		Прокси = МодульПолучениеФайловИзИнтернетаКлиентСервер.ПолучитьПрокси(СтруктураURI.Схема);
	КонецЕсли;
