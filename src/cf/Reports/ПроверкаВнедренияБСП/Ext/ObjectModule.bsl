					...
					Если Отсутствующие.Количество() > 0 Тогда
						...
						Иначе
							...
							Если ЗначениеЗаполнено(Пояснение) Тогда
								Подробно = Пояснение + Символы.ПС + Подробно;
							КонецЕсли;
						КонецЕсли;
...
Процедура ПроверитьСоставРазделителей()
	...
	ИменаXMLВыгрузки = Новый Структура;
	ИменаXMLВыгрузки.Вставить("ПланОбмена", "ExchangePlan");
	ИменаXMLВыгрузки.Вставить("РегламентноеЗадание", "ScheduledJob");
	ИменаXMLВыгрузки.Вставить("Константа", "Constant");
	ИменаXMLВыгрузки.Вставить("Справочник", "Catalog");
	ИменаXMLВыгрузки.Вставить("Документ", "Document");
	ИменаXMLВыгрузки.Вставить("ПланВидовХарактеристик", "ChartOfCharacteristicTypes");
	ИменаXMLВыгрузки.Вставить("ПланСчетов", "ChartOfAccounts");
	ИменаXMLВыгрузки.Вставить("ПланВидовРасчета", "ChartOfCalculationTypes");
	ИменаXMLВыгрузки.Вставить("РегистрСведений", "InformationRegister");
	ИменаXMLВыгрузки.Вставить("РегистрНакопления", "AccumulationRegister");
	ИменаXMLВыгрузки.Вставить("РегистрБухгалтерии", "AccountingRegister");
	ИменаXMLВыгрузки.Вставить("РегистрРасчета", "CalculationRegister");
	ИменаXMLВыгрузки.Вставить("БизнесПроцесс", "BusinessProcess");
	ИменаXMLВыгрузки.Вставить("Задача", "Task");
		...
		Для Каждого СтрокаСостава Из МетаданныеРазделителя.Состав Цикл
			...
			Если (ПлановоеЗначениеОтличаетсяОтАвто И ФактическоеЗначениеСовпадаетСАвто)
				...
				Если ИсправлятьОшибку("НекорректныйСоставРазделителей") Тогда
					...
					ПолноеИмя = ИменаXMLВыгрузки[ЧастиИмени[0]] + "." + ЧастиИмени[1];
...
Процедура ПроверитьСоставСтандартныхРолей()
	...
	ИменаXMLВыгрузки = Новый Соответствие;
	ИменаXMLВыгрузки.Вставить("Константа", "Constant");
	ИменаXMLВыгрузки.Вставить("РегистрРасчета", "CalculationRegister");
	ИменаXMLВыгрузки.Вставить("РегистрСведений", "InformationRegister");
	ИменаXMLВыгрузки.Вставить("РегистрНакопления", "AccumulationRegister");
	...
	ИменаXMLВыгрузки.Вставить("БизнесПроцесс", "BusinessProcess");
	ИменаXMLВыгрузки.Вставить("Задача", "Task");
	ИменаXMLВыгрузки.Вставить("Последовательность", "Sequence");
	ИменаXMLВыгрузки.Вставить("ЖурналДокументов", "DocumentJournal");
	...
	Для Каждого ЭлементСостава Из Метаданные.ОбщиеРеквизиты.Найти("ОбластьДанныхОсновныеДанные").Состав Цикл
		...
		ЧастиИмени = СтрРазделить(ПолноеИмя, ".");
		ТипОбъекта = ИменаXMLВыгрузки[ЧастиИмени[0]];
...
Процедура ОбновитьСписокПредопределенныхЭлементовСправочника(ИмяСправочника, СписокПредопределенных, РабочийКаталог)
	...
	Если ИмяСправочника = "ИдентификаторыОбъектовМетаданных" Тогда
		Для Каждого ОписаниеПредопределенного Из ПредопределенныеИдентификаторыВсехРегистров() Цикл
			ИмяПредопределенного = ОписаниеПредопределенного.Ключ;
			ОбъектМетаданных = Метаданные.НайтиПоПолномуИмени(ОписаниеПредопределенного.Значение);
...
Функция СоответствиеИменОбъектов()
...
Процедура ЗаполнитьДанныеДляПроверки(ПараметрыПроверки)
	...
	СоответствиеТерминов = СоответствиеИменОбъектов();
